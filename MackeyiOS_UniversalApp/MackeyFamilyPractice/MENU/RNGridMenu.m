...
//        _titleLabel.font=  [UIFont fontWithName:@"Arial" size:28.0f];
...
- (void)layoutSubviews {
    ...
    if (hasText) {
        ...
        if (hasImage) {
            ...
            height = floorf(CGRectGetHeight(frame) / 8.f);
        }
...
- (instancetype)initWithItems:(NSArray *)items {
    if ((self = [super init])) {
        ...
        {
            _itemSize = CGSizeMake(300.f, 300.f);
...
//        _backgroundColor = [UIColor colorWithWhite:0.5 alpha:0.7];
        _bounces = YES;
...
- (void)styleItemViews {
    [self.itemViews enumerateObjectsUsingBlock:^(RNMenuItemView *itemView, NSUInteger idx, BOOL *stop) {
        ...
        if([deviceType isEqualToString:@"iPhone"] || [deviceType isEqualToString:@"iPod touch"])
        {
            itemView.titleLabel.font =[UIFont fontWithName:@"Arial" size:14.0f];
        }else
        {
           itemView.titleLabel.font =[UIFont fontWithName:@"Arial" size:28.0f];
        }
...
- (void)selectItemViewAtPoint:(CGPoint)point {
    ...
    if (![item isEmpty]) {
        ...
        [self performSelector:@selector(removeBackground) withObject:nil afterDelay:2];
